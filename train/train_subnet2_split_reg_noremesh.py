...
parser.add_argument('--model', type=str,default='./log/SVR_subnet2_usage3/network.pth',help='model path from the trained subnet1')
...
parser.add_argument('--env', type=str, default="SVR_subnet2_4_split", help='visdom env')
...
parser.add_argument('--lambda_edge', type=float, default=1e-5) # Default : 1e-5
...
network.cuda()
'''
network.encoders2[0] = network.encoders2[0].cuda()
network.encoders2[1] = network.encoders2[1].cuda()
network.encoders2[2] = network.encoders2[2].cuda()
network.encoders2[3] = network.encoders2[3].cuda()
network.encoders2[4] = network.encoders2[4].cuda()
network.encoders2[5] = network.encoders2[5].cuda()
network.encoders2[6] = network.encoders2[6].cuda()
network.encoders2[7] = network.encoders2[7].cuda()
'''
'''
{'params': network.encoders2[0].parameters()},
{'params': network.encoders2[1].parameters()},
{'params': network.encoders2[2].parameters()},
{'params': network.encoders2[3].parameters()},
{'params': network.encoders2[4].parameters()},
{'params': network.encoders2[5].parameters()},
{'params': network.encoders2[6].parameters()},
{'params': network.encoders2[7].parameters()},
'''
lrate = opt.lr
...
for epoch in range(opt.nepoch):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        loss_net = cds_stage2 + opt.lambda_edge * edge_loss + l2_loss + opt.lambda_normal * normal_loss + 0.1 * vol_l1_loss
        ...
        train_CDs_stage2_sd_loss.update(0.0)
        ...
        now = datetime.datetime.now()
        print(
            '[%s] / [%d: %d/%d] train_cd_loss:  %f, train_cd_loss_sd:  %f, l2_loss: %f, edge_loss: %f, normal_loss: %f, vol_l1_loss: %f, loss_net: %f'
            % (
            now, epoch, i, len(dataset) / opt.batchSize, cds_stage2.item(), 0.0, 0.0, edge_loss.item(),
            normal_loss.item(), 0.0, loss_net.item()))
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(dataloader_test, 0):
            ...
            val_CDs_stage2_sd_loss.update(0.0)
            ...
            now = datetime.datetime.now()
            print(
                '[%s] / [%d: %d/%d] val_cd_loss:  %f, val_cd_loss_sd:  %f, l2_loss: %f, edge_loss: %f, normal_loss: %f, vol_l1_loss: %f, loss_net: %f'
                % (now, epoch, i, len(dataset_test) / opt.batchSize, cds_stage2.item(), 0.0, 0.0, edge_loss.item(), 0.0,
                   normal_loss.item(), loss_net.item()))
