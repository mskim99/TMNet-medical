...
def get_boundary(faces):
    ...
    repeated_index = np.array(list(set(all_index).difference(set(unique_index)))).astype(int)
...
def samples_random(faces_cuda, pointsRec, sampled_number):
    ...
    if len(faces_cuda.size())==2:
        faces_points = pointsRec.index_select(1, faces_cuda.reshape(-1)).contiguous().\
